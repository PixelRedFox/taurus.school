 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".swiper-new-car")) new core(".swiper-new-car", {
                ...
                pagination: {
                    el: ".swiper-new-car-pagination",
                    type: "bullets",
                    clickable: true,
                    dynamicBullets: true
                },
                navigation: {
                    prevEl: ".swiper-new-car-button-prev",
                    nextEl: ".swiper-new-car-button-next"
                }
            });
            if (document.querySelector(".swiper-video-review")) new core(".swiper-video-review", {
                modules: [ Navigation, Pagination ],
                slidesPerView: 4,
                spaceBetween: 25,
                breakpoints: {
                ...
                pagination: {
                    el: ".swiper-review-student-pagination",
                    type: "bullets",
                    clickable: true,
                    dynamicBullets: true
                },
                navigation: {
                    prevEl: ".swiper-review-student-button-prev",
                    nextEl: ".swiper-review-student-button-next"
                }
            });
        }
        ...
        window.onload = function() {
            const endTime = (new Date).getTime() + 8e3 * 60 * 60 * 12;
            const promo = document.querySelector("#promoBar");
            const closePromoBtn = document.querySelector("#promoCloseBtn");
            closePromoBtn.addEventListener("click", closePromo);
            function closePromo() {
                setCookie("sPromoBar", "true", 4);
                promo.style.display = "none";
                promo.innerHTML = "";
            }
            if (!getCookie("sPromoBar")) if (getCookie("timePromoExpire")) doExpires(promo.querySelector("#expDate"), getCookie("timePromoExpire")); else {
                doExpires(promo.querySelector("#expDate"), endTime);
                setCookie("timePromoExpire", endTime, 4);
            } else promo.style.display = "none";
            function doExpires(el, endTime) {
                setInterval((function() {
                    const now = new Date;
                    const timeLeft = endTime - now.getTime();
                    el.innerHTML = timeString(timeLeft);
                }), 1e3);
            }
            ...
            function timeString(exp) {
                const days = Math.floor(exp / (1e3 * 60 * 60 * 24)), hours = Math.floor(exp % (1e3 * 60 * 60 * 24) / (1e3 * 60 * 60)), minutes = Math.floor(exp % (1e3 * 60 * 60) / (1e3 * 60)), seconds = Math.floor(exp % (1e3 * 60) / 1e3), plural = days > 1 ? "дні" : "день", daysStr = days ? days + " " + plural + " " : "", hoursStr = hours ? padNum(hours) + " <span class='dd'>:</span> " : "", minsStr = minutes ? padNum(minutes) + " <span class='dd'>:</span> " : "";
            ...
            new ClipboardJS(cpnBtn, {
                text: function() {
                    return cpnCode.innerHTML;
                }
            });
            cpnBtn.addEventListener("click", (function() {
                cpnBtn.innerHTML = `скопійовано`;
                setTimeout((function() {
                    cpnBtn.innerHTML = "копіювати";
                }), 3e3);
            }));
            const tabsM = document.querySelectorAll(".tab-link-m");
            const contentsM = document.querySelectorAll(".tab-content-m");
            const tabsA = document.querySelectorAll(".tab-link-a");
            const contentsA = document.querySelectorAll(".tab-content-a");
            const addTabClickHandler = (tabs, contents) => {
            ...
            addTabClickHandler(tabsM, contentsM);
            addTabClickHandler(tabsA, contentsA);
            const handleInputChange = event => {
                const input = event.target;
                if (input.value.length > 0) input.classList.add("filled"); else input.classList.remove("filled");
            };
            const textInputs = document.querySelectorAll(".lead-form__input");
            textInputs.forEach((input => {
                input.addEventListener("input", handleInputChange);
            }));
            function trackFormSubmitButton(forms) {
                forms.forEach((form => {
                    ...
                    if (submitButton) {
                        submitButton.addEventListener("click", (event => {
                            ...
                            for (let i = 0; i < inputs.length; i++) {
                                ...
                                switch (name) {
                                  case "name":
                                    ...
                                    !isValid ? error.classList.add("_active") : error.classList.remove("_active");
                                    break;
                                  ...
                                  case "phone":
                                    isValid = /^\+38 \(\d{3}\) \d{3}-\d{2}-\d{2}$/.test(value);
                                    !isValid ? error.classList.add("_active") : error.classList.remove("_active");
                                    break;
                            ...
                            if (validationStatus.includes(false)) return; else {
                                form.classList.add("sending");
                                submitButton.innerText = "Надсилання ...";
                                const hiddenFields = form.querySelectorAll('input[type="hidden"]');
                                hiddenFields.forEach((field => {
                                    if (field.name.includes("utm_")) {
                                        const urlParams = new URLSearchParams(window.location.search);
                                        const utmValue = urlParams.get(field.name);
                                        if (utmValue) field.value = utmValue;
                                    } else field.value = document.title;
                                }));
                                setTimeout((() => {
                                    form.submit();
            ...
            const formsSubmit = document.querySelectorAll(".lead-form");
            trackFormSubmitButton(formsSubmit);
            class PhoneInputMask {
                ...
                handleEvent(event) {
                    ...
                    const reg = new RegExp(`^${matrix.substr(0, this.phoneInput.value.length).replace(/_+/g, (a => `\\d{1,${a.length}}`)).replace(/[+()]/g, "\\$&")}$`);
                    if (!reg.test(this.phoneInput.value) || this.phoneInput.value.length < 5 || this.keyCode > 47 && this.keyCode < 58) this.phoneInput.value = newValue;
                    if ("blur" === event.type && (this.phoneInput.value === matrix || this.phoneInput.value.length < 6)) this.phoneInput.value = "";
                }
                attachEventListeners() {
                    [ "input", "focus", "blur", "keydown", "mouseup" ].forEach((eventType => {
                        this.phoneInput.addEventListener(eventType, this.handleEvent);
                    }));
                }
            }
            document.querySelectorAll(".phone-input").forEach((phoneInput => {
                new PhoneInputMask(phoneInput);
            }));
        };
        window["FLS"] = true;
        isWebp();
